# -*- coding: utf-8 -*-
# Define here the models for your scraped Extensions
import logging
import time
from scrapy import signals
from scrapy.exceptions import NotConfigured
import caixin.settings
from caixin.spiders.caixincrawl import CaixincrawlSpider
import redis

logger = logging.getLogger(__name__)

#这个代码的框架其实之前已经写好了，我还在想你们怎么这么厉害
class RedisSpiderSmartIdleClosedExensions(object):

    def __init__(self, idle_number, crawler):
        self.crawler = crawler
        self.idle_number = idle_number
        self.idle_list = []
        self.idle_count = 0
    # 下面这个函数的固定的，里面的步骤基本是一致的
    @classmethod
    def from_crawler(cls, crawler):
        # 首先检查是否应该启用和提高扩展
        # 否则不配置
        if not crawler.settings.getbool('MYEXT_ENABLED'):
            raise NotConfigured

        # 这里是获取set中定义的自己定义的参数
        # 获取配置中的时间片个数，默认为360个，30分钟
        idle_number = crawler.settings.getint('IDLE_NUMBER', 360)

        # 实例化扩展对象
        ext = cls(idle_number, crawler)

        # 将扩展对象连接到信号， 将signals.spider_idle 与 spider_idle() 方法关联起来。
        crawler.signals.connect(ext.spider_opened, signal=signals.spider_opened)
        crawler.signals.connect(ext.spider_closed, signal=signals.spider_closed)
        crawler.signals.connect(ext.spider_idle, signal=signals.spider_idle)

        # return the extension object
        return ext

    def spider_opened(self, spider):
        logger.info("opened spider %s redis spider Idle, Continuous idle limit： %d", spider.name, self.idle_number)

    def spider_closed(self, spider):
        logger.info("closed spider %s, idle count %d , Continuous idle count %d",
                    spider.name, self.idle_count, len(self.idle_list))

    def spider_idle(self, spider):
        # 这个对象里面的属性得以保留的原因是这个对象被实例化了，并不是我之前理解的这仅仅只是一个函数而已
        # 没有想清楚这个是怎么触发的,原来是所有声明为spider_idle的函数在消息队列为0之后都会被调用
        print('close_spider.spider_idle has been called!')
        self.idle_count += 1  # 空闲计数
        # print(self.idle_count)
        # print(time.time())
        self.idle_list.append(time.time())  # 每次触发 spider_idle时，记录下触发时间戳
        idle_list_len = len(self.idle_list)  # 获取当前已经连续触发的次数

        # 判断 当前触发时间与上次触发时间 之间的间隔是否大于5秒，如果大于5秒，说明redis 中还有key
        # 根据之前的判断空的原理可以知道，如果队列中是空的了，那么下一次触发与上一次触发的时间间隔
        # 一定是5s，如果大于5s，就说明从上一次触发之后，又有链接被送到了队列中，那么我只需要将最新的时间戳保存下来就可以了
        if idle_list_len > 2 and self.idle_list[-1] - self.idle_list[-2] > 6:
            self.idle_list = [self.idle_list[-1]]

        elif idle_list_len > self.idle_number:
            # 连续触发的次数达到配置次数后关闭爬虫
            logger.info('\n continued idle number exceed {} Times'
                        '\n meet the idle shutdown conditions, will close the reptile operation'
                        '\n idle start time: {},  close spider time: {}'.format(self.idle_number,
                                                                                self.idle_list[0], self.idle_list[0]))
            # 执行关闭爬虫操作
            # print('CaixincrawlSpider.article_count'+str(CaixincrawlSpider.article_count))
            # print('CaixincrawlSpider.artinformation_count'+str(CaixincrawlSpider.artinformation_count))
            self.crawler.engine.close_spider(spider, 'closespider_pagecount')
            # if caixin.settings.CLEANREDIS:
            #     try:
            #         rconn_filter = redis.Redis(caixin.settings.REDIS_HOST, caixin.settings.REDIS_PORT,
            #                                    caixin.settings.REDIS_DB)
            #     except Exception:
            #         try:
            #             rconn_filter = redis.Redis(caixin.settings.REDIS_HOST, caixin.settings.REDIS_PORT)
            #         except Exception:
            #             rconn_filter = None
            #
            #     print(rconn_filter.keys())
            #     # if rconn:
            #     #     if 'SinaSpider:requests' in rconn.keys():
            #     #         rconn.delete('SinaSpider:requests')
            #
            #     if rconn_filter:
            #         if 'bloomfilter0' in str(rconn_filter.keys()):
            #             print('found')
            #             rconn_filter.delete('bloomfilter0')
            #     #     if 'SinaSpider:dupefilter1' in rconn_filter.keys():
            #     #         rconn_filter.delete('SinaSpider:dupefilter1')
            #     print(rconn_filter.keys())
            #     print('clean redis Finish!')